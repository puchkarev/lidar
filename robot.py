...
if __name__ == '__main__':
  ...
  def update(frame):
    ...
    # Determine if we are moving or staying put
    ...
    if show_real_lidar:
      ...
      ax.plot([robot.position[0] + d * numpy.cos(numpy.deg2rad(a) + robot.position[2]) for i, a, d in lidar_data], \
              [robot.position[1] + d * numpy.sin(numpy.deg2rad(a) + robot.position[2]) for i, a, d in lidar_data], \
              'bo', markersize=3)
    ...
    if show_mapped_position_mean:
      ax.plot([robot_mean[0]], [robot_mean[1]], 'ro', markersize=4)
